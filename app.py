        ...
        for ing in ingredients:
            if ing['sub_recipe_id']:
                ...
                for sub_ing in sub_ingredients:
                    scaled_ing = dict(sub_ing); scaled_ing['quantity'] = float(scaled_ing.get('quantity', 0)) * scaling_ratio; base_ingredients.append(scaled_ing)
            elif ing['inventory_item_id']:
                raw_ing = {'inventory_item_id': ing['inventory_item_id'], 'name': ing['inv_name'], 'unit': ing['inv_unit'], 'quantity': float(ing.get('quantity', 0))}
...
# --- CORRECTED create_recipe ---
@app.route('/create', methods=['POST'])
def create_recipe():
    conn = get_db_connection(); recipe_id = None
    try:
        with conn.cursor() as cur:
            # ... (Recipe insertion code remains the same) ...
            ...
            # ... (Ingredient insertion loop remains the same) ...
        ...
        # Return inside the try block after successful commit
        return redirect(url_for('recipe_dashboard'))

    except (psycopg2.Error, ValueError) as e:
        if conn: conn.rollback() # Ensure rollback happens on error
        flash(f"Error creating recipe: {e}", "error")
        print(f"Error creating recipe: {e}")
        # Fetch data for re-rendering form
        ...
        # Return inside the except block to show errors on the form page
        return render_template('add_recipe.html', recipes=recipes, inventory_items=inventory_items)
    finally: # Finally block is now correctly indented
        if conn: conn.close() # Ensure connection is closed


# --- CORRECTED update_recipe ---
@app.route('/update/<int:recipe_id>', methods=['POST'])
def update_recipe(recipe_id):
    conn = get_db_connection()
    try:
        with conn.cursor() as cur:
            # ... (Recipe update code remains the same) ...
...
def wip_batch_detail(batch_id):
    ...
    with conn.cursor(cursor_factory=DictCursor) as cur:
        ...
        else: batches_needed_float = 1.0
        for ing in base_ingredients_one_batch:
            inv_item_id = ing.get('inventory_item_id')
            if inv_item_id: name = ing.get('name', 'Unknown').strip(); unit = ing.get('unit', 'Unknown').strip(); key = (inv_item_id);
            required_ingredients[key]['name'] = name; required_ingredients[key]['unit'] = unit; required_ingredients[key]['inventory_item_id'] = inv_item_id; required_ingredients[key]['total_needed'] += float(ing.get('quantity', 0)) * batches_needed_float
    ...
    conn.close(); return render_template('wip_batch_detail.html', batch=batch, ingredient_summary=ingredient_summary, inventory_items=available_inventory_items)
...
def allocate_ingredient(batch_id):
    ...
    try:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            if result is None: flash("Selected inventory item not found.", "error"); conn.rollback()
            elif result['available'] < quantity: flash(f"Not enough available stock (Available: {result['available']:.2f}). Allocation failed.", "error"); conn.rollback()
            else: cur.execute("INSERT INTO wip_allocations (wip_batch_id, inventory_item_id, quantity_allocated) VALUES (%s, %s, %s);",(batch_id, inventory_item_id, quantity)); cur.execute("UPDATE inventory_items SET quantity_allocated = quantity_allocated + %s WHERE id = %s;", (quantity, inventory_item_id)); conn.commit(); flash("Ingredient allocated successfully.", "success")
    except psycopg2.Error as e: conn.rollback(); flash(f"Database error during allocation: {e}", "error"); print(f"DB Error allocating to batch {batch_id}: {e}")
    finally:
        if conn: conn.close()
...
def delete_wip_batch(batch_id):
    ...
    try:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            elif batch_status['status'] == 'Completed': flash("Cannot delete a completed batch.", "error"); conn.rollback()
            else:
