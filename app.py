import os
import psycopg2
from psycopg2.extras import DictCursor, RealDictCursor  # Import RealDictCursor
            ...
            if ing["sub_recipe_id"]:
                ...
                if sub_recipe_yield_row:
                    ...
                    if yield_unit in ("grams", "mLs"):
                        ...
                        # Use direct batch scaling logic
    ...
    try:
        ...
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            for recipe in recipes_from_db:
                ...
                cur.execute(
                    """SELECT i.quantity, i.sub_recipe_id, COALESCE(inv.name, r_sub.name, i.name, 'Unknown') as name, COALESCE(inv.unit, i.unit, 'N/A') as unit FROM ingredients i LEFT JOIN inventory_items inv ON i.inventory_item_id = inv.id LEFT JOIN recipes r_sub ON i.sub_recipe_id = r_sub.id WHERE i.recipe_id = %s;""",
                    (recipe["id"],),
                )
...
def new_recipe_form():
    ...
    return render_template(
        "add_recipe.html", recipes=recipes, inventory_items=inventory_items
    )
    ...
    try:
        with conn.cursor() as cur:
            ...
            for i in range(len(quantities)):
                inventory_item_id = (
                    inventory_item_ids[i] if inventory_item_ids[i] else None
                )
                ...
                if not inventory_item_id and not sub_recipe_id:
                    flash(f"Row {i+1} skipped.", "warning")
                    continue
                ...
                if inventory_item_id:
                    ...
                    cur.execute(
                        "SELECT name FROM recipes WHERE id = %s;", (sub_recipe_id,)
                    )
                    ...
                    if sub_recipe_data:
                        ...
                        item_unit = "batch"
                cur.execute(
                    """INSERT INTO ingredients (recipe_id, inventory_item_id, sub_recipe_id, quantity, name, unit) VALUES (%s, %s, %s, %s, %s, %s);""",
                    (
                        recipe_id,
                        inventory_item_id,
                        sub_recipe_id,
...
def products_page():
    ...
    try:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute(
                """SELECT p.id, p.sku, p.product_name, p.jars_per_batch, r.name as recipe_name FROM products p LEFT JOIN recipes r ON p.recipe_id = r.id ORDER BY p.product_name, p.sku;"""
            )
...
def stock_minimums_page():
    ...
    try:
        if request.method == "POST":
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO stock_minimums (location_id, product_id, min_jars) VALUES (%s, %s, %s) ON CONFLICT (product_id, location_id) DO UPDATE SET min_jars = EXCLUDED.min_jars;""",
        ...
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            locations = cur.fetchall()
            cur.execute(
                """SELECT p.id, p.sku, p.product_name, r.name as recipe_name FROM products p JOIN recipes r ON p.recipe_id = r.id WHERE r.is_sold_product = TRUE ORDER BY p.product_name, p.sku;"""
            )
            products = cur.fetchall()
            cur.execute(
                """SELECT sm.id, l.name as location_name, p.sku, p.product_name, r.name as recipe_name, sm.min_jars FROM stock_minimums sm JOIN locations l ON sm.location_id = l.id JOIN products p ON sm.product_id = p.id JOIN recipes r ON p.recipe_id = r.id ORDER BY l.name, p.sku;"""
            )
...
def inventory_log():
    """
    Displays a log of all manual inventory adjustments, with filtering.
    """
    conn = get_db_connection()
    adjustments = []
    inventory_items = []
    # Get filter_item_id from query params, ensure it's an integer if present
    ...
    try:
        ...
        # Fetch all inventory items for the filter dropdown
        ...
        query_sql = """
            ...
            JOIN 
                inventory_items ii ON ia.inventory_item_id = ii.id
        """
